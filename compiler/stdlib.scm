...
(define fold-left
 	(let ((null? null?)
 	(car car) (cdr cdr))
 		(letrec ((fold-left-loop (lambda (f init seq)
 			(if (null? seq)
 				init
 				(fold-left-loop f
 										(f init (car seq))
...
(define fold-right
 	(let ((null? null?)
 	(car car) (cdr cdr))
 		(letrec ((fold-right-loop (lambda (f init seq)
 			(if (null? seq)
 				init
 				(f (car seq) (fold-right-loop f init (cdr seq)))))))
 							fold-right-loop)))
...
(define cons*
	(let ((null? null?)
	(car car) (cdr cdr))
		(letrec ((cons*-loop (lambda (first . rest)
			(if (null? rest)
				first
				(cons first (apply cons*-loop rest))))))
					cons*-loop)))
